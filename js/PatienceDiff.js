 ...
 * version: 2.0
 *
 * !! This code was modified and no longer works as intended by the author !!
 * !! the original file can be found on https://github.com/jonTrent/PatienceDiff !!
 *
...
function patienceDiff(aLines, bLines, only_one_result=false) {
  ...
  function addToResult(aIndex, bIndex) {
    ...
    if (bIndex < 0) {
        result.push({row: aIndex-deleted+inserted, type: "delete", old_text: aLines[aIndex], text:""});
        deleted++;
    } else if (aIndex < 0) {
        var first = true;
        for (c of result) {
          if (c.row == bIndex) {
            if (first) {
              c.type = "change";
              c.text = bLines[bIndex];
              if (c.text == c.old_text) {
                  c.type = "nothing";
              }
              first = false;
            } else {
          ...
          } else if (c.row > bIndex) {
              c.row += 1;
          }
        }
        if (first) {
            result.push({row: bIndex, type: "insert", text: bLines[bIndex]});
        }
        inserted++;
    }
  ...
  function addSubMatch(aLo, aHi, bLo, bHi) {
    if (only_one_result && result.length != 0) return;
